...
export async function requestPrinterConnection(
  vendorId?: number | null,
  productId?: number | null,
  ...
  const device = await navigator.usb.requestDevice({
    filters: getZebraLikeFilter(vendorId, productId),
  });
  if (!device.opened) {
    await device.open();
  }
  ...
  const selectedInterface = configuration.interfaces.find((item) =>
    item.alternates.some((alternate) =>
...
export async function sendRawPayload(
  ...
  const result = await connection.device.transferOut(
