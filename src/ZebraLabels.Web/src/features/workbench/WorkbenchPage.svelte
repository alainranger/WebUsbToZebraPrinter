...
<script lang="ts">
	...
	import type { ApiProblem, LabelTemplateDetails, PrinterProfile, PreviewOutputType, PreparedPrintPayload } from '../../lib/api/types';
	import { browserSupportsWebUsb, closePrinterConnection, requestPrinterConnection, sendRawPayload, type PrinterConnection } from '../../lib/webusb/printer';
	...
	const selectedPrinter = $derived(printerProfiles.find((profile) => profile.id === selectedPrinterProfileId) ?? null);

	async function loadData() {
		problem = null;
		try {
			const summaries = await apiClient.listTemplates();
			templates = await Promise.all(summaries.map((template) => apiClient.getTemplate(template.id)));
	...
	async function generatePreview() {
		...
		try {
			const result = await apiClient.renderTemplate(selectedTemplate.id, outputType, variableValues);
	...
	async function connectPrinter() {
		...
		try {
			usbConnection = await requestPrinterConnection(selectedPrinter.vendorId, selectedPrinter.productId);
			...
			<div class="grid two">
				...
				<div class="card">
					...
					<p class:warning={!webUsbSupported}>
						{webUsbSupported ? 'WebUSB available in this browser.' : 'WebUSB unavailable in this browser.'}
					</p>
					{#if usbConnection}
						<p class="muted">Connected to {usbConnection.device.productName ?? 'selected USB device'}.</p>
					{/if}
				</div>
			</div>
